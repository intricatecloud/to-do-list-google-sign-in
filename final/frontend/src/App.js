...
const getToken = () => {
  return new Promise((resolve, reject) => {
    gapi.load('auth2', () => {
      try {
        console.log('loaded auth2 from get-token')
...
function ToDoList() {
  ...
  const getTodoList = () => {
    getToken().then(token => {
      ...
      return axios.get(`${host}/todo`, {
        headers: {
          Authorization: token,
        }
      })
    }).then(res => {
      console.log('setting list', res.data)
      setList(res.data)
    })
  }
  ...
  const createNewListItem = () => {
    ...
    getToken().then(token => {
      return axios.post(`${host}/todo`, {item: newListItem}, { headers: { 'Authorization': token }})
    }).then(() => {
      setNewListItem("")
      return getTodoList()
    })
...
function App() {
  ...
  const onGoogleSignInLoaded = () => {
    console.log('on google sign in loaded')
    gapi.load('auth2', () => {
